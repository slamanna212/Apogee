    ...
    controls
        .attach(move |event: MediaControlEvent| {
            let payload = match event {
                MediaControlEvent::Play => Some(MediaEventPayload {
                    kind: "play",
                    value: None,
                }),
                MediaControlEvent::Pause => Some(MediaEventPayload {
                    kind: "pause",
                    value: None,
                }),
                MediaControlEvent::Toggle => Some(MediaEventPayload {
                    kind: "toggle",
                    value: None,
                }),
                MediaControlEvent::SetVolume(v) => Some(MediaEventPayload {
                    kind: "volume",
                    value: Some(v),
                }),
...
#[cfg(target_os = "linux")]
#[tauri::command]
pub fn media_session_set_volume(
    state: State<'_, MediaSessionState>,
    volume: f64,
) -> Result<(), String> {
    let mut guard = state.0.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn media_session_set_volume(
    _state: State<'_, MediaSessionState>,
    _volume: f64,
) -> Result<(), String> {
    Ok(())
}
